...
int main(int argc, char **argv) {
	...
	switch (update_used | use_used << 1 | install_used << 2 | 0 << 3) {
	case 0b0001:
		update();
		break;
	case 0b0010:
		for (auto pkg :
			 install_command.get<std::vector<std::string>>("packages"))
			use_internal(pkg);
		break;
	case 0b0100:
		install_packages(
		...
		break;
	case 0b1000:
		// query(search_command.get<std::vector<std::string>>("queries"));
		break;
	}
	// update();
